...
namespace nandina::theme
{
    namespace
    {
        ...
        [[nodiscard]] auto state_tint_amount(const NanTheme& theme, ButtonVisualState state)
            -> float {
            switch (state) {
    ...
    auto button_color_pair(const NanTheme& theme, ButtonTone tone) -> ButtonColorPair {
        switch (tone) {
            ...
            case ButtonTone::neutral:
                return {
                    .accent = theme.palette.surface_variant,
                    .on_accent = theme.palette.on_surface_variant
                };
    ...
    auto resolve_button_style(
        ...
        switch (treatment) {
            ...
            case ButtonTreatment::outlined:
                style.background = tint > 0.0F ? theme.palette.surface.mix(pair.accent, tint)
                                               : transparent(theme.palette.surface);
                style.foreground = pair.accent;
