...
#ifndef NANDINA_EXPERIMENT_WIDGET_RADIO_BUTTON_HPP
#define NANDINA_EXPERIMENT_WIDGET_RADIO_BUTTON_HPP
...
{
    class RadioButton: public primitives::Pressable {
    public:
        explicit RadioButton(
            std::string label,
            std::shared_ptr<RadioGroup> group = nullptr,
        ...
        /// 用户/语义激活：进入互斥组后取消其它选中项。
        void select();

        /// 加入/退出互斥组（nullptr 表示独立单选项）。
        void set_group(std::shared_ptr<RadioGroup> group);
        [[nodiscard]] auto group() const -> const std::shared_ptr<RadioGroup>&;
        ...
        void set_override(theme::RadioButtonRecipeRule rule);
    ...
    private:
        ...
        void emit_if_changed(bool previous);
        ...
        std::optional<theme::RadioButtonRecipeRule> override_;
...
#endif // NANDINA_EXPERIMENT_WIDGET_RADIO_BUTTON_HPP
