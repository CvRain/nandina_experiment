...
namespace nandina::widget
{
    namespace
    {
        ...
        [[nodiscard]] auto
        cell_constraints(scene::LayoutConstraints parent, float cell_width) -> scene::LayoutConstraints {
            return {
                .min_width = 0.0F,
    ...
    auto Grid::on_measure(const scene::LayoutConstraints constraints) -> foundation::NanSize {
        ...
        const int row_count =
            (total_children + columns_ - 1) / columns_; // ceil division
        ...
        const float available_width =
            std::max(0.0F, constraints.max_width - total_column_gap);
        ...
        for (int i = 0; i < total_children; ++i) {
            ...
            const auto measured = children[static_cast<std::size_t>(i)]->measure_layout(
                child_constraints
            );
    ...
    auto Grid::on_layout() -> void {
        ...
        for (int i = 0; i < total_children; ++i) {
            const int row = i / columns_;
            const auto child_constraints = cell_constraints(
                scene::LayoutConstraints::loose(),
                cell_width
            );
            const auto measured = children[static_cast<std::size_t>(i)]->measure_layout(
                child_constraints
        ...
        for (int row = 0; row < row_count; ++row) {
            ...
            float x = 0.0F;
            const int cols_in_row =
                std::min(columns_, total_children - row * columns_);
