...
public:
    ...
    [[nodiscard]] auto device() -> IRenderDevice& { return device_; }

    /// Current node's world transform (written by _propagate_draw on the way down).
    [[nodiscard]] auto world_transform() const -> const NanTransform2D& { return world_; }

    /// Inherited opacity in [0,1]: parent opacity x this node's opacity.
    [[nodiscard]] auto opacity() const -> float { return opacity_; }

    /// Clip stack (RAII push/pop).
    [[nodiscard]] auto clip() -> ClipStack& { return clip_; }
...
[[nodiscard]] inline auto world_bounds_from_local(const NanTransform2D& xf, const NanRect& local)
    -> NanRect {
