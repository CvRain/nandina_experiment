...
public:
    void begin_frame() override { BeginDrawing(); }
    ...
    void set_clip(const NanRect& r) override {
        ...
        BeginScissorMode(static_cast<int>(r.get_left()), static_cast<int>(r.get_top()),
                         static_cast<int>(r.get_width()), static_cast<int>(r.get_height()));
    }

    void clear_clip() override { EndScissorMode(); }
    ...
    void draw_rounded_rect(const NanRect& r, float radius, const NanColor& c) override {
        ...
        const float roundness = shortest > 0.0F
            ? std::clamp(radius * 2.0F / shortest, 0.0F, 1.0F) : 0.0F;
        DrawRectangleRounded(to_rl(r), roundness, 8, to_rl(c));
    }

    void draw_line(const NanPoint& a, const NanPoint& b, float thickness,
                   const NanColor& c) override {
    ...
    void draw_text(std::string_view text, const NanPoint& pos, float font_size,
                   const NanColor& c) override {
        const std::string owned(text);
        DrawText(owned.c_str(), static_cast<int>(pos.get_x()), static_cast<int>(pos.get_y()),
                 static_cast<int>(font_size), to_rl(c));
    }
};
