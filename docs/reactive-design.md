...
3. 最外层 `batch` 结束后统一 flush。
4. 同一个 invalidator 在同一批次中至多执行一次。
5. flush 期间若出现异常，会在必要的清理完成后重抛。
...
- 支持 effect / computed invalidator 去重
...
依赖追踪仍然基于 `thread_local current_invalidator`，因此当前默认语义仍是单线程 UI 状态模型。`batch(...)` 也只保证当前线程中的批量 flush 一致性，不承诺跨线程事务行为。
