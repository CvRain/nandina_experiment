...
> 状态：实现完成，聚焦测试与 43/43 全量自动化通过；C5.4 增加异步首次加载。
...
C5.4 在同一纹理键和驻留边界上增加 pending 请求合并：CPU 解码/预处理在后台线程执行，RGBA
纹理上传回到 UI 线程，完成后仍进入本节定义的 LRU。详细线程模型与后续 I/O/可见性优化见
`ASYNC_IMAGE_LOADING.md`。
...
异步 package I/O、可见区域懒加载、每帧上传预算、glyph atlas dirty-region 更新、预热清单、
跨窗口共享、热重载和详细运行时显存面板留给后续单元；它们不改变本文件定义的所有权边界。
