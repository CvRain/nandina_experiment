...
> 状态：C5 Linux 平台门禁已完成。C5.1 窗口指标/DPI、C5.2 系统偏好、C5.3 资源驻留、
> C5.4 异步图片和 C5.7 路径/关闭流程均已通过自动化与双设备实机验收。本文记录 Linux 1.0
> 的真实边界，不代表 Windows 或 macOS 支持。
...
双设备实机记录覆盖桌面会话、输入、资源加载、触摸板交互和 teardown。
...
### 6.2 2026-09-05 第二台设备记录

- 设备：CachyOS 笔记本电脑。
- 会话：Hyprland 与 Plasma 均完成验证。
- 结果：按本节清单完成窗口、DPI/resize、主题与动效、CJK 输入、应用内外剪贴板、Gallery
  异步图片、页面资源复用和 teardown 验收，未发现回归。
- 额外观察：笔记本触摸板能够正常驱动点击、滚动和页面交互，属于额外通过的输入路径。
