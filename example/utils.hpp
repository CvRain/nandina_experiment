...
[[nodiscard]] inline auto to_raylib(const foundation::NanColor& color) -> Color {
    const auto [red, green, blue, alpha] = color.to<foundation::NanHexRgb>();
    return {
...
[[nodiscard]] inline auto from_raylib(const Color color) -> foundation::NanColor {
    return foundation::NanColor::from(
        foundation::NanHexRgb {.red = color.r, .green = color.g, .blue = color.b, .alpha = color.a}
    );
}

[[nodiscard]] inline auto lighten(const Color color, int amount) -> Color {
    auto bump = [amount](const unsigned char channel) -> unsigned char {
...
[[nodiscard]] inline auto lerp_color(const Color a, const Color b, float t) -> Color {
    const auto lerp = [t](const unsigned char from, const unsigned char to) -> unsigned char {
...
public:
    ...
    void on_draw(render::DrawContext& ctx) override {
        ...
        const auto shadow =
            from_raylib(lerp_color(current_fill_color(), BLACK, 0.28F)).with_alpha(0.16F * a);
        ...
        device.draw_rect(world, fill);
            ...
            if (focused_) {
                const auto ring = foundation::NanRect::from_xywh(
                    world.get_left() - 3.0F,
                    world.get_top() - 3.0F,
                    world.get_width() + 6.0F,
                    world.get_height() + 6.0F
                );
                device.draw_rect_outline(
                    ring,
                    2.0F,
            ...
            if (!label_.empty()) {
                ...
                const auto text_color =
                    from_raylib(hovered_ ? BLACK : Color {32, 34, 42, 255}).with_alpha(a);
...
inline auto build_deep_chain(Color color, float dx, float dy, const char* prefix) -> DeepChain {
...
inline auto build_transform_rig(foundation::NanPoint center, Color primary, Color accent)
    -> TransformRig {
    TransformRig rig;
...
inline auto build_chain_rig(foundation::NanPoint center, Color chain_color, Color accent)
    -> ChainRig {
    ChainRig rig;
...
inline auto build_stack_rig(foundation::NanPoint center, Color low, Color mid, Color high)
    -> StackRig {
    StackRig rig;
...
inline void draw_panel(float x, float y, float w, float h, Color bg, Color border) {
    DrawRectangle(
        static_cast<int>(x),
        static_cast<int>(y),
        static_cast<int>(w),
