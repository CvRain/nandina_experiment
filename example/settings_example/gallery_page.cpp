...
namespace nandina::examples::settings
{
    ...
    auto GalleryPage::build(app::PageContext& context) -> std::shared_ptr<scene::NanNode2D> {
        auto ui = context.ui();
        const auto placeholder = ui.theme().palette.surface_variant;
        const auto gallery_image =
            [&ui,
             placeholder](std::string source, std::string title, foundation::NanSize upload_size) {
                auto image = ui.make<widget::Image>(std::move(source))
                                 .configure([upload_size, placeholder](widget::Image& image) {
                                     ...
                                     image.set_placeholder_color(placeholder);
        ...
        for (std::size_t index = 0; index < samples.size(); ++index) {
            content->add_child(
                gallery_image(samples[index].first, samples[index].second, upload_sizes[index])
            );
        }
