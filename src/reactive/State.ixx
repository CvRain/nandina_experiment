...
export namespace Nandina {
    ...
    // ── State<T> ──────────────────────────────────────────────────────────────
    // Reactive value holder. Reading inside an Effect auto-registers the Effect
    // as an observer; writing notifies all observers.
    //
    // State is intentionally non-movable after construction (ReadState stores a
    // raw pointer to it). Use State exclusively as a named member variable.
    //
    // Subscribe to value changes from outside the Effect system:
    ...
    public:
        ...
        /// Subscribe to value changes. The callback is invoked with the new
        /// value each time set() succeeds (i.e., value actually changed).
        /// Returns a Connection; call disconnect() to unsubscribe.
    ...
    private:
        ...
        auto notify() -> void {
            // Move effect-observers out so Effects can re-register during re-run.
            ...
            // Also notify on_change subscribers (persistent, not re-registered).
